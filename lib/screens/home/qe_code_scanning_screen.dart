...
class _QrScanScreenState extends State<QrScanScreen>
  ...
  Future<void> _checkInitialConnection() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _isOnline = true; // Default to online if error
        });
      }
    }
  }
  ...
  void dispose() {
    ...
    _connectionBarTimer?.cancel(); // ✅ Timer cancel karo
  ...
  void _resetScreenState() {
    if (mounted) {
      setState(() {
        ...
        _isHandlingConnection = false;
        _isOnline = true;
        _showConnectionBar = false; // ✅ Status bar hide
        _connectionBarText = "";
        _connectionBarColor = Colors.transparent;
      });
    }
...
//   // 🔴 NEW: iOS ke liye important flag
//   bool _isProcessingLogout = false;
...
//         if (_wasOffline) {
//           _wasOffline = false;
//           if (mounted && !_isProcessingLogout) {
...
//   /// 🔴 UPDATED: Verify customer with iOS fix
//   Future<void> verifyCustomer(String customerId) async {
//     if (isFetching || _isProcessingLogout) return;
...
//   /// 🔴 UPDATED: Token expired handling for iOS
//   Future<void> _handleTokenExpired() async {
//     if (_isProcessingLogout) return;

//     // 🔴 IMPORTANT: Pehle existing overlay ko properly clear karo
//     if (mounted) {
//       setState(() {
//         showErrorOverlay = false;
//         errorSubtitle = "";
...
//     // 🔴 IMPORTANT: iOS ko state update ka time do
...
//   /// 🔴 UPDATED: Connection restored handling
...
//       setState(() {
//         showErrorOverlay = true;
//         errorSubtitle = "Connection restored. Please login again to continue.";
//       });
//     }
//   }

//   /// 🔴 UPDATED: Main fix - Error dialog with iOS compatibility
//   void _showErrorDialog(String message, {bool showLoginButton = false}) {
//     if (_isProcessingLogout) return;

//     // 🔴 IMPORTANT: Pehle existing overlay clear karo
//     if (mounted) {
//       setState(() {
//         showErrorOverlay = false;
//       });
...
//     // 🔴 IMPORTANT: iOS ko time do state update ke liye
...
//   /// 🔴 UPDATED: Logout method with iOS fix
//   Future<void> _performLogout() async {
//     if (_isProcessingLogout) return;

//     _isProcessingLogout = true; // 🔴 IMPORTANT: Flag set karo

//     // 1. Pehle UI states clear karo
//     if (mounted) {
//       setState(() {
//         isFetching = false;
//         showErrorOverlay = false; // 🔴 IMPORTANT: Overlay hide karo
//         errorSubtitle = "";
//         qrResult = null;
//       });
//     }

//     // 2. iOS ko time do state update ke liye
//     await Future.delayed(const Duration(milliseconds: 100));
...
//     // 4. iOS ke liye additional delay
//     if (defaultTargetPlatform == TargetPlatform.iOS) {
//       await Future.delayed(const Duration(milliseconds: 200));
//     }

//     // 5. Navigation with proper cleanup
//     if (mounted) {
//       // 🔴 IMPORTANT: Get.offAll ki jagah Navigator use karo iOS ke liye
//       Navigator.of(context).pushAndRemoveUntil(
//         MaterialPageRoute(builder: (context) => const LoginScreen()),
//         (Route<dynamic> route) => false,
//       );
...
//                       onPressed: () async {
//                         Navigator.of(context).pop();
//                         await _performLogout(); // 🔴 Updated method call
//                       },
//                       child: const Text("OK", style: TextStyle(fontSize: 16)),
//                     ),
//                   ],
...
//   // 🔴 UI CODE BILKUL SAME HAI - KOI CHANGE NAHI
//   @override
//   Widget build(BuildContext context) {
//     final double boxWidth = MediaQuery.of(context).size.width * 0.80;
//     final double boxHeight = MediaQuery.of(context).size.height * 0.35;
...
//                           text:
//                               errorSubtitle.contains("expired") ||
//                                   errorSubtitle.contains("Connection restored")
//                               ? "Login Again"
//                               : "Scan More",
//                           onPressed: () async {
...
//       // App foreground mein aayi hai - token check karo
//       _checkTokenOnResume();
//     }
//   }

//   void _setupIOSLifecycle() {
//     // Additional iOS-specific initialization if needed
//   }
...
//           Get.to(() => UserDetailScreen(userData: data["data"]));
//         } else {
//           _showErrorDialog(data["message"] ?? "User not found");
//         }
//       } else if (response.statusCode == 401) {
//         await _handleTokenExpired();
//       } else {
...
//   Future<void> _handleTokenExpired() async {
//     // Pehle existing overlay ko clear karein
//     if (mounted) {
//       setState(() {
//         showErrorOverlay = false;
//         errorSubtitle = "";
...
//     // Thoda delay dekar naya overlay show karein (iOS ke liye important)
//     await Future.delayed(const Duration(milliseconds: 100));
...
//   void _showErrorDialog(String message, {bool showLoginButton = false}) {
//     // Pehle existing overlays clear karein
//     if (mounted) {
//       setState(() {
//         showErrorOverlay = false;
//       });
...
//     // Thoda wait karein state update ke liye
//     Future.delayed(const Duration(milliseconds: 50), () {
//       if (mounted) {
//         setState(() {
//           showErrorOverlay = true;
...
//       // Auto-dismiss only for non-login cases
//       if (!showLoginButton) {
//         Future.delayed(const Duration(seconds: 3), () {
//           if (mounted) {
//             setState(() => showErrorOverlay = false);
...
//     // SharedPreferences clear karein
//     SharedPreferences prefs = await SharedPreferences.getInstance();
//     await prefs.clear();

//     // iOS ke liye additional cleanup
//     if (defaultTargetPlatform == TargetPlatform.iOS) {
//       await Future.delayed(const Duration(milliseconds: 100));
//     }

//     // Navigation with proper cleanup
//     if (mounted) {
